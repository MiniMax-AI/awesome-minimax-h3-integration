...
| **32 GB (RTX 5090), lossless** | [SGLang verified recipe](https://docs.sglang.ai/cookbook/diffusion/MiniMax/MiniMax-H3#hw=rtx5090&host_ram=ram32&weights=fl2va&mode=t2va&placement=offload&attention=platform&precision=native&encoder=auto&execution=eager&quality=lossless&outputs=1&nodes=1&gpus_per_node=1&topology_mode=auto&tp_size=1&ulysses_degree=1&ring_degree=1) | No quantization: native-precision FL2VA on a single 5090 via layerwise offload of DiT + TE + VAE (`--performance-mode memory`, 20 resident DiT layers). Verified end-to-end at a **32 GB host-RAM** baseline (~250 s/request); 48–64 GB host RAM pins weights and drops this to ~180–218 s. Set `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`. |
| **RTX 50-series / Blackwell in ComfyUI** | [NVIDIA Sol-Attn](https://github.com/kijai/ComfyUI-SolAttn_triton) | **1.14–1.44×** faster than SageAttention with **−37 %** MLP peak VRAM, measured on a 5090. SM89–SM121, Triton 3.6.0. |
...
| :--- | :--- |
| [SGLang](https://github.com/sgl-project/sglang) | First-class H3 serving. SGLang Diffusion ships a native H3 pipeline with an official [cookbook](https://docs.sglang.ai/cookbook/diffusion/MiniMax/MiniMax-H3): `sglang serve --model-variant fl2va\|ref2va` covers t2va, fl2va, and ref2va (V2V is a ref2va use case), with every published recipe verified on real hardware — 4×H200/H100, B200/B300 (online quantization), [single RTX 5090 at native precision](https://docs.sglang.ai/cookbook/diffusion/MiniMax/MiniMax-H3#hw=rtx5090&host_ram=ram32&weights=fl2va&mode=t2va&placement=offload&attention=platform&precision=native&encoder=auto&execution=eager&quality=lossless&outputs=1&nodes=1&gpus_per_node=1&topology_mode=auto&tp_size=1&ulysses_degree=1&ring_degree=1) (layerwise offload, 32 GB host-RAM baseline) as well as 2×RTX 5090, single RTX 4090 24 GB (`kitchen_int8`), and day-0 AMD MI355X/MI300X via ROCm + AITER. Also the selectable backend in [`MiniMax-H3-ComfyUI`](https://github.com/MiniMaxH3ComfyUI/MiniMax-H3-ComfyUI). |
